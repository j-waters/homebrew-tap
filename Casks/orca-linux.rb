cask "orca-linux" do
  arch arm: "-arm64"

  version "1.4.204"
  sha256 arm64_linux:  "337efc5afa65d40bfd853f7ca42f2c0c9501a914e87c38ce4c47f12a59a16c8e",
         x86_64_linux: "0c8481057d250bcd06299214bd5e21439137964b28e68cc2ea176f9a0ebc3559"

  url "https://github.com/stablyai/orca/releases/download/v#{version}/orca-linux#{arch}.AppImage"
  name "Orca"
  desc "IDE for orchestrating AI coding agents across terminals and worktrees"
  homepage "https://onorca.dev/"

  livecheck do
    url :url
    strategy :github_latest
  end

  # Why: electron-updater (src/main/updater.ts) handles in-place updates the
  # same way it does on macOS, this time rewriting the AppImage itself. That
  # keeps `brew upgrade` a no-op unless --greedy, matching the upstream cask.
  auto_updates true
  depends_on :linux

  app_image "orca-linux#{arch}.AppImage", target: "Orca.AppImage"
  # Why: a stable, arch-independent launch path. app_image moves the real
  # AppImage to ~/Applications/Orca.AppImage on every (re)install, so this
  # symlink survives upgrades even though the versioned Caskroom path behind
  # it doesn't. install-steps blocks may only contain literal step calls - no
  # interpolated Ruby - so `{{HOMEBREW_PREFIX}}` (a supported template token)
  # is the only way to embed an absolute launch path in the .desktop file
  # below; `#{Dir.home}` isn't available there the way it is here.
  binary "#{Dir.home}/Applications/Orca.AppImage", target: "orca"

  # Why: app_image only moves the AppImage into ~/Applications - unlike macOS
  # Cask, brew's Linux app_image stanza has no Launch Services equivalent, so
  # nothing registers a menu entry or icon on its own. Unlike kitty/warp (real
  # archives Cask auto-unpacks, so their artifact stanzas just point at files
  # already on disk), an AppImage stays one opaque file - its icon has to be
  # pulled out via its own --appimage-extract fallback (no FUSE/root needed)
  # before it can be referenced. This block's default base is :staged_path,
  # so chdir "." and the copy source below land in/read from the version-
  # scoped Caskroom directory, not $HOME.
  #
  # Why the extraction is now full (no icon-only pattern) and kept instead of
  # removed: Orca's own CLI registration (Settings -> Install CLI) writes
  # ~/.local/bin/orca-ide as a bash script with an absolute, version-pinned
  # path to *this* staged AppImage baked in - it can't be pointed at
  # ~/Applications/Orca.AppImage instead, and its generator lives inside
  # Orca's closed-source app.asar, not here. On a host with no usable FUSE
  # device, mounting that AppImage fails ("Cannot mount AppImage, please
  # check your FUSE setup."), and `--appimage-extract-and-run` alone
  # re-extracts and prints the whole file listing to stdout on *every*
  # invocation, corrupting CLI output. So this extracts once, here, next to
  # $APPIMAGE; the postflight step below installs an orca-ide shim that runs
  # squashfs-root/orca-ide directly.
  # Keying this to staged_path means brew prunes it for free when a
  # version's Caskroom directory goes away on uninstall/upgrade, and a
  # reinstall regenerates it.
  postflight_steps do
    run "Applications/Orca.AppImage",
        base:  :home,
        args:  ["--appimage-extract"],
        chdir: "."

    # Why: Orca's generated CLI wrapper executes the AppImage directly, which
    # requires FUSE. Point the installed shim at the extracted CLI instead so
    # `orca-ide ...` works on hosts without a usable FUSE device. The staged
    # path is version-scoped and is regenerated on install/upgrade.
    write_file ".local/bin/orca-ide", <<~SH, base: :home
      #!/usr/bin/env bash
      set -euo pipefail
      runtime='{{staged_path}}/squashfs-root/orca-ide'
      if [ ! -x "$runtime" ]; then
        echo "Orca extracted CLI not found at $runtime; reinstall the orca-linux cask." >&2
        exit 1
      fi
      exec "$runtime" "$@"
    SH
    set_permissions ".local/bin/orca-ide", "0755", base: :home

    copy "squashfs-root/usr/share/icons/hicolor/512x512/apps/orca-ide.png",
         ".local/share/icons/hicolor/512x512/apps/orca.png",
         target_base: :home

    write_file ".local/share/applications/orca.desktop", <<~DESKTOP, base: :home
      [Desktop Entry]
      Version=1.0
      Type=Application
      Name=Orca
      Comment=IDE for orchestrating AI coding agents across terminals and worktrees
      Exec="{{HOMEBREW_PREFIX}}/bin/orca" %U
      Icon=orca
      Categories=Development;
      StartupNotify=true
      StartupWMClass=orca
      Keywords=ai;coding;agent;claude;codex;
    DESKTOP
  end

  uninstall_postflight_steps do
    remove ".local/share/applications/orca.desktop", base: :home
    remove ".local/share/icons/hicolor/512x512/apps/orca.png", base: :home
  end

  # Why: mirrors the upstream cask's zap list. Orca writes worktree/agent
  # state under ~/.orca regardless of platform; ~/.config and ~/.cache are
  # Electron's standard Linux userData/cache dirs (the counterparts to
  # ~/Library/Application Support and ~/Library/Caches on macOS).
  zap trash: [
    "~/.cache/Orca",
    "~/.config/Orca",
    "~/.orca",
  ]
end
